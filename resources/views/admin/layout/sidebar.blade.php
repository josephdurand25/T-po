    ...
    >
      ...
      <div>
        ...
        <ul class="mb-6 flex flex-col gap-1.5">
            ...
            {{-- {{ dd($vues) }} --}}
            {{-- @forelse ($vues as $vue )
              <x-link class="{{ (Str::startsWith(Route::currentRouteName() , 'admin.'.$vue->view_folder)) ? 'bg-graydark dark:bg-meta-4': '' }}" route="{{ 'admin.'.$vue->view_folder.'.index' }}" icon="{{ $vue->ico_view }}" >
                {{ $vue->view_folder }}
              </x-link>
              @empty
